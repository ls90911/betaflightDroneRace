...
# 1 "<command-line>"
# 1 "./src/main/sensors/gyroanalyse.c"
# 26 "./src/main/sensors/gyroanalyse.c"
...
# 27 "./src/main/sensors/gyroanalyse.c" 2
...
# 32 "./src/main/sensors/gyroanalyse.c" 2
...
#pragma GCC diagnostic pop
# 39 "./src/main/sensors/gyroanalyse.c" 2
...
# 64 "./src/main/sensors/gyroanalyse.c"
static uint16_t __attribute__ ((section(".fastram_bss"), aligned(4))) fftSamplingRateHz;

static uint16_t __attribute__ ((section(".fastram_bss"), aligned(4))) fftBpfHz;

static float __attribute__ ((section(".fastram_bss"), aligned(4))) fftResolution;

static uint16_t __attribute__ ((section(".fastram_bss"), aligned(4))) dynNotchMaxCentreHz;
static uint8_t __attribute__ ((section(".fastram_bss"), aligned(4))) fftBinOffset;


static __attribute__ ((section(".fastram_bss"), aligned(4))) float hanningWindow[32];
static __attribute__ ((section(".fastram_bss"), aligned(4))) float dynamicNotchCutoff;

void gyroDataAnalyseInit(uint32_t targetLooptimeUs)
{
# 87 "./src/main/sensors/gyroanalyse.c"
    const int gyroLoopRateHz = lrintf((1.0f / targetLooptimeUs) * 1e6f);



    fftSamplingRateHz = __extension__ ({ __typeof__ ((gyroLoopRateHz / 3)) _a = ((gyroLoopRateHz / 3)); __typeof__ (1333) _b = (1333); _a < _b ? _a : _b; });

    fftBpfHz = fftSamplingRateHz / 4;
    fftResolution = (float)fftSamplingRateHz / 32;
    dynNotchMaxCentreHz = fftSamplingRateHz / 2;




    fftBinOffset = __extension__ ({ __typeof__ (1) _a = (1); __typeof__ (lrintf(90 / fftResolution - 1.5f)) _b = (lrintf(90 / fftResolution - 1.5f)); _a > _b ? _a : _b; });

    for (int i = 0; i < 32; i++) {
        hanningWindow[i] = (0.5f - 0.5f * cos_approx(2 * 3.14159265358979323846f * i / (32 - 1)));
    }
...
void gyroDataAnalyseStateInit(gyroAnalyseState_t *state, uint32_t targetLooptimeUs)
{
    ...
    arm_rfft_fast_init_f32(&state->fftInstance, 32);




    const float looptime = __extension__ ({ __typeof__ (1000000u / fftSamplingRateHz) _a = (1000000u / fftSamplingRateHz); __typeof__ (targetLooptimeUs * (3 * 4)) _b = (targetLooptimeUs * (3 * 4)); _a > _b ? _a : _b; });
    for (int axis = 0; axis < 3; axis++) {
        ...
        biquadFilterInitLPF(&state->detectedFrequencyFilter[axis], 60, looptime);
    }
}
...
void gyroDataAnalyse(gyroAnalyseState_t *state, biquadFilter_t *notchFilterDyn)
{
    ...
    if (state->sampleCount == state->maxSampleCount) {
        ...
        for (int axis = 0; axis < 3; axis++) {
            ...
            if (axis == 0) {
                {if (debugMode == (DEBUG_FFT)) {debug[(2)] = (lrintf(sample));}};
            }

            state->oversampledGyroAccumulator[axis] = 0;
        }

        state->circularBufferIdx = (state->circularBufferIdx + 1) % 32;


        state->updateTicks = (3 * 4);
    }
...
static void gyroDataAnalyseUpdate(gyroAnalyseState_t *state, biquadFilter_t *notchFilterDyn)
{
    ...
    {if (debugMode == (DEBUG_FFT_TIME)) {debug[(0)] = (state->updateStep);}};
    switch (state->updateStep) {
        case STEP_ARM_CFFT_F32:
        {
            ...
            case 64:
                ...
                arm_radix8_butterfly_f32(state->fftData, (32 / 2), Sint->pTwiddle, 1);
                break;
            }
        ...
        case STEP_ARM_CMPLX_MAG_F32:
        {
            ...
            arm_cmplx_mag_f32(state->rfftData, state->fftData, (32 / 2));
...
# 258 "./src/main/sensors/gyroanalyse.c" 3 4
           _Bool 
# 258 "./src/main/sensors/gyroanalyse.c"
                fftIncreasing = 
# 258 "./src/main/sensors/gyroanalyse.c" 3 4
                                0
# 258 "./src/main/sensors/gyroanalyse.c"
            ...
            for (int i = 1 + fftBinOffset; i < (32 / 2); i++) {
                ...
                if (fftIncreasing || data > prevData * 2) {
                    ...
                    if (!fftIncreasing) {
                        ...
                        fftIncreasing = 
# 272 "./src/main/sensors/gyroanalyse.c" 3 4
                                       1
# 272 "./src/main/sensors/gyroanalyse.c"
                                           ;
                    }
            ...
            if (fftSum > 0) {
                ...
                centerFreq = constrain(fftMeanIndex * fftResolution, 125, dynNotchMaxCentreHz);
            }

            centerFreq = biquadFilterApply(&state->detectedFrequencyFilter[state->updateAxis], centerFreq);
            centerFreq = constrain(centerFreq, 125, dynNotchMaxCentreHz);
            state->centerFreq[state->updateAxis] = centerFreq;

            if (state->updateAxis == 0) {
               {if (debugMode == (DEBUG_FFT)) {debug[(3)] = (lrintf(fftMeanIndex * 100));}};
            }
            {if (debugMode == (DEBUG_FFT_FREQ)) {debug[(state->updateAxis)] = (state->centerFreq[state->updateAxis]);}};
            {if (debugMode == (DEBUG_FFT_TIME)) {debug[(1)] = (micros() - startTime);}};
            break;
        }
        case STEP_UPDATE_FILTERS:
        {
            ...
            const float cutoffFreq = fmax(state->centerFreq[state->updateAxis] * dynamicNotchCutoff, 105);
            ...
            state->updateAxis = (state->updateAxis + 1) % 3;
            state->updateStep++;
            __attribute__ ((fallthrough));
        }
        case STEP_HANNING:
        {
            ...
            const uint8_t ringBufIdx = 32 - state->circularBufferIdx;
